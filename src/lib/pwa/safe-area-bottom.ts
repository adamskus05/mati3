...
export function applySafeBottomLock(px?: number): number {
  ...
  root.style.removeProperty("--mati-app-height");
...
export function safeAreaBottomBootScript(): string {
  return `(function(){try{var r=document.documentElement,s=window.matchMedia('(display-mode: standalone)').matches,k='${STORAGE_KEY}',n=null;r.style.removeProperty('--mati-app-height');try{var st=sessionStorage.getItem(k);if(st!=null){var p=parseInt(st,10);if(!isNaN(p)&&p>=${MIN_PX}&&p<=${MAX_PX})n=p}}catch(e){}if(n===null){var d=document.createElement('div');d.style.cssText='position:fixed;bottom:0;left:0;padding-bottom:env(safe-area-inset-bottom);visibility:hidden;pointer-events:none';r.appendChild(d);var px=parseFloat(getComputedStyle(d).paddingBottom)||0;r.removeChild(d);var m=Math.round(px);n=s?(m>=12&&m<=${MAX_PX}?m:${PWA_SAFE_BOTTOM_FALLBACK_PX}):0}r.style.setProperty('${CSS_VAR}',n+'px');try{sessionStorage.setItem(k,String(n))}catch(e){}}catch(e){}})();`;
}
